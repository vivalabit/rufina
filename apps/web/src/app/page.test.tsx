...
it("reports an unfinished provider snapshot without claiming zero results", async () => {
  window.history.replaceState(null, "", "#jobs");
  const entryItConfig = {
    id: "entry-it",
    name: "Entry IT",
  ...
  const fetchMock = vi.fn<typeof fetch>(async (input, init) => {
    ...
    if (url.pathname === "/job-search/run" && method === "POST") {
      return Response.json({
        ...
        jobsAnalyzed: 0,
        screeningErrors: 0,
        jobsScreeningAiCalls: 0,
        sourceErrors: {
          linkedin:
            "linkedin snapshot snapshot-1 is still running after 600s; no results were downloaded yet",
        },
    ...
    if (url.pathname === "/job-search/runs" && method === "GET") {
      return Response.json([]);
    }
    if (
      (url.pathname === "/jobs" ||
  ...
  expect(
    screen.queryByText(
      "LinkedIn search finished: 0 found, 0 matched config, 0 added",
    ),
  ).not.toBeInTheDocument();
});
...
it("adds and removes LinkedIn professions without dropping supporting queries", async () => {
  window.history.replaceState(null, "", "#jobs");
  const savedFilters: Array<Record<string, unknown>> = [];
  const commonConfig = {
    id: "entry-it",
    name: "Entry IT",
    createdAt: "2026-07-21T00:00:00.000Z",
  ...
  const fetchMock = vi.fn<typeof fetch>(async (input, init) => {
    ...
    if (url.pathname === "/job-search/source-configs" && method === "GET") {
      return Response.json([sourceConfig]);
    }
    if (
      url.pathname === "/job-search/source-configs/entry-it-linkedin" &&
      method === "PATCH"
    ) {
      const body = JSON.parse(String(init?.body)) as {
        filters: Record<string, unknown>;
      };
      savedFilters.push(body.filters);
      return Response.json({
        ...sourceConfig,
        ...body,
  ...
  expect(
    screen.getByText(
      /1 supporting entry-level search term stays active automatically\./,
    ),
  ).toBeInTheDocument();
  fireEvent.click(
  ...
  fireEvent.change(screen.getByLabelText("New LinkedIn profession"), {
  ...
  fireEvent.click(
    screen.getByRole("button", { name: "Save LinkedIn query config" }),
  );
  await waitFor(() => expect(savedFilters).toHaveLength(1));

  const savedQueries = savedFilters[0].linkedinQueries as Array<{
    keyword: string;
    experienceLevels: string[];
  }>;
  expect(savedQueries.map((query) => query.keyword)).toEqual([
    "Data Analyst",
    "junior",
    "Security Analyst",
  ]);
  expect(
    savedQueries.find((query) => query.keyword === "Security Analyst"),
  ).toMatchObject({
    experienceLevels: ["Entry level", "Internship"],
  });
});
...
it("uses a broad direction and full-catalog filters for a direct-company-only search", async () => {
  window.history.replaceState(null, "", "#jobs");
  const runBodies: Array<Record<string, unknown>> = [];
  const entryItConfig = {
    id: "entry-it",
...
it("shows direct-company vacancies with their company logos", async () => {
  ...
  const fetchMock = vi.fn<typeof fetch>(async (input, init) => {
    ...
    if (url.pathname === "/job-search/run" && method === "POST") {
      ...
      const komaxGroupJob = importedJobData({
        id: "komax_group-2157",
      ...
      const juliusBaerSwitzerlandJob = importedJobData({
        id: "julius_baer_switzerland-r-19350",
        title: "Head of Group External Communications at Julius Baer",
        source: "julius_baer_switzerland",
      });
      const bkwSwitzerlandJob = importedJobData({
        id: "bkw_switzerland-974955bb-343b-44e5-bfa9-af77587ef35d",
        title: "Finance Systems Analyst FP&A at BKW",
        source: "bkw_switzerland",
      });
      const snbJob = importedJobData({
        id: "snb-3453",
      ...
      const bayerSwitzerlandJob = importedJobData({
        id: "bayer_switzerland-562949978313882",
        title: "Plant Engineer at Bayer",
        source: "bayer_switzerland",
      });
      const biogenSwitzerlandJob = importedJobData({
        id: "biogen_switzerland-REQ23932",
      ...
      const pwcSwitzerlandJob = importedJobData({
        id: "pwc_switzerland-00000000-0000-4000-8000-000000000001",
        title: "Consultant at PwC Switzerland",
        source: "pwc_switzerland",
      });
      const txGroupJob = importedJobData({
        id: "tx_group-8172175",
        title: "Lehre als Kauffrau / Kaufmann EFZ at TX Group",
        source: "tx_group",
      });
      const artificialyJob = importedJobData({
        id: "artificialy-4440007645",
      ...
      const blueworksJob = importedJobData({
        id: "blueworks-https-join-com-companies-blue-15934805",
      ...
      const bossInfoJob = importedJobData({
        id: "boss_info-JOB-1166",
      ...
      const datahouseZurichJob = importedJobData({
        id: "datahouse_zurich-744000141111111-role-0",
        title: "System Architect at Datahouse",
        source: "datahouse_zurich",
      });
      const esuranceJob = importedJobData({
        id: "esurance-https-esurance-jobs-personio-com-job-2759674",
        title: "Agile Operations Lead at esurance",
        source: "esurance",
      });
      const frankeSwitzerlandJob = importedJobData({
        id: "franke_switzerland-1428534833",
        title: "CX-Solution Project Manager at Franke",
        source: "franke_switzerland",
      });
      const helgaJob = importedJobData({
        id: "helga-senior-backend-developer-drupal",
        title: "Senior Drupal Developer at Helga",
        source: "helga",
      });
      const helsanaJob = importedJobData({
        id: "helsana-71324d82-0065-4246-9fce-b490e927ad89",
      ...
      storedJobs = [
        ...
        {
          id: juliusBaerSwitzerlandJob.id,
          data: juliusBaerSwitzerlandJob,
        },
        ...
        { id: datahouseZurichJob.id, data: datahouseZurichJob },
        { id: esuranceJob.id, data: esuranceJob },
  ...
  expect(screen.getByText("Gilead Sciences Switzerland")).toBeInTheDocument();
  ...
  expect(screen.getByText("GSK Switzerland")).toBeInTheDocument();
  expect(screen.getByText("Sanofi Switzerland")).toBeInTheDocument();
  expect(screen.getByText("Takeda Switzerland")).toBeInTheDocument();
  expect(screen.getByText("Johnson & Johnson Switzerland")).toBeInTheDocument();
  expect(screen.getByText("Zurich Insurance")).toBeInTheDocument();
  expect(
    screen.getByText("Schneider Electric Switzerland"),
  ).toBeInTheDocument();
  expect(screen.getByText("Bosch Switzerland")).toBeInTheDocument();
  expect(screen.getByText("Eviden Switzerland")).toBeInTheDocument();
  expect(screen.getByText("Schindler Group")).toBeInTheDocument();
  expect(screen.getByText("Sonova Group")).toBeInTheDocument();
  expect(screen.getByText("Elektro-Material AG")).toBeInTheDocument();
  expect(screen.getByText("CSL Switzerland")).toBeInTheDocument();
  expect(screen.getByText("Lonza Switzerland")).toBeInTheDocument();
  expect(screen.getByText("Nestlé Switzerland")).toBeInTheDocument();
  expect(screen.getByText("AFRY Switzerland")).toBeInTheDocument();
  expect(screen.getByText("Digital Realty Switzerland")).toBeInTheDocument();
  expect(screen.getByText("Equans Switzerland")).toBeInTheDocument();
  expect(screen.getByText("Bison Group")).toBeInTheDocument();
  expect(screen.getByText("PRODYNA Switzerland")).toBeInTheDocument();
  expect(screen.getByText("cross-ING Switzerland")).toBeInTheDocument();
  expect(screen.getByText("NetApp Switzerland")).toBeInTheDocument();
  expect(screen.getByText("Sharp Switzerland")).toBeInTheDocument();
  expect(screen.getByText("Unisys Switzerland")).toBeInTheDocument();
  expect(screen.getByText("R&M Switzerland")).toBeInTheDocument();
  expect(screen.getByText("AbbVie Switzerland")).toBeInTheDocument();
  expect(screen.getByText("Lyreco Switzerland")).toBeInTheDocument();
  expect(screen.getByText("Adcubum Switzerland")).toBeInTheDocument();
  ...
  expect(screen.getByText("Datahouse Zurich")).toBeInTheDocument();
  expect(screen.getByText("esurance AG")).toBeInTheDocument();
  ...
  expect(screen.getByText("Intersim AG")).toBeInTheDocument();
  expect(screen.getByText("kaiko.ai Zürich")).toBeInTheDocument();
  expect(screen.getByText("Karakun AG")).toBeInTheDocument();
  expect(screen.getByText("LAKE Solutions AG")).toBeInTheDocument();
  expect(screen.getByText("Operaio GmbH")).toBeInTheDocument();
  expect(screen.getByText("Rey Technology")).toBeInTheDocument();
  expect(screen.getByText("SCHURTER AG")).toBeInTheDocument();
  expect(screen.getByText("smino AG")).toBeInTheDocument();
  expect(
  ...
  expect(screen.getByText("Direct companies (177)")).toBeInTheDocument();
  fireEvent.click(screen.getByRole("checkbox", { name: /SBB CFF FFS/ }));
  ...
  expect(runRequests[0]).toMatchObject({
    sources: [
      ...
      "nestle_switzerland",
      "afry_switzerland",
      "digital_realty_switzerland",
      "equans_switzerland",
      "bison_group",
      "prodyna_switzerland",
      "crossing_switzerland",
      "netapp_switzerland",
      "sharp_switzerland",
      "unisys_switzerland",
      "rdm_switzerland",
  ...
  expect(
    screen.getAllByRole("img", { name: "Julius Baer Switzerland logo" }).length,
  ).toBeGreaterThan(0);
  expect(
    screen.getAllByRole("img", { name: "BKW Switzerland logo" }).length,
  ).toBeGreaterThan(0);
  expect(
  ...
  expect(
    screen.getAllByRole("img", { name: "Bayer Switzerland logo" }).length,
  ).toBeGreaterThan(0);
  expect(
  ...
  expect(
    screen.getAllByRole("img", { name: "PwC Switzerland logo" }).length,
  ).toBeGreaterThan(0);
  expect(
    screen.getAllByRole("img", { name: "TX Group AG logo" }).length,
  ).toBeGreaterThan(0);
  expect(
  ...
  expect(
    screen.getAllByRole("img", { name: "Datahouse Zurich logo" }).length,
  ).toBeGreaterThan(0);
  expect(
    screen.getAllByRole("img", { name: "esurance AG logo" }).length,
  ).toBeGreaterThan(0);
  expect(
    screen.getAllByRole("img", { name: "Franke Switzerland logo" }).length,
  ).toBeGreaterThan(0);
  expect(
    screen.getAllByRole("img", { name: "Helga Digitalagentur logo" }).length,
  ).toBeGreaterThan(0);
  expect(
  ...
  expect(
    screen.getAllByText("Source: Julius Baer Switzerland").length,
  ).toBeGreaterThan(0);
  expect(screen.getAllByText("Source: BKW Switzerland").length).toBeGreaterThan(
    0,
  );
  expect(
  ...
  expect(
    screen.getAllByText("Source: Bayer Switzerland").length,
  ).toBeGreaterThan(0);
  expect(
  ...
  expect(screen.getAllByText("Source: PwC Switzerland").length).toBeGreaterThan(
    0,
  );
  expect(screen.getAllByText("Source: TX Group AG").length).toBeGreaterThan(0);
  ...
  expect(
    screen.getAllByText("Source: Datahouse Zurich").length,
  ).toBeGreaterThan(0);
  expect(screen.getAllByText("Source: esurance AG").length).toBeGreaterThan(0);
  expect(
    screen.getAllByText("Source: Franke Switzerland").length,
  ).toBeGreaterThan(0);
  expect(
