...
def upsert_applications(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="An application changed during legacy batch upsert",
        ) from exc
    except SQLAlchemyError as exc:
        db.rollback()
        raise HTTPException(
    ...
    try:
        for event in request.events:
            ...
            if record:
                ...
                db.add(
                    StoredApplicationEventRecord(
                        id=event.id, application_id=event.application_id, data=event.data
                    )
                )
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="An application event changed during legacy batch upsert",
        ) from exc
    except SQLAlchemyError as exc:
        db.rollback()
        raise HTTPException(
