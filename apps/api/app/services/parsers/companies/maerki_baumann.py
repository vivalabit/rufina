...
MAERKI_BAUMANN_CAREERS_URL = "https://www.maerki-baumann.ch/de/unsere-bank/stellenangebote"
MAERKI_BAUMANN_API_URL = (
    "https://odm.ostendis.com/ojp/data/v55/jobs/"
    "58c324307ff846428a19a2f36b7f9994/DE"
    "?domain=www.maerki-baumann.ch"
)
MAERKI_BAUMANN_HEADERS = {
...
EXPECTED_COMPANY = "Maerki Baumann & Co. AG"


class MaerkiBaumannParseError(DirectCompanyRequestError):
    pass


class MaerkiBaumannJobsParser:
    """Collect the complete official Maerki Baumann Ostendis catalog."""

    parser_id = "maerki_baumann"

    def __init__(
        self,
        *,
        base_url: str = MAERKI_BAUMANN_CAREERS_URL,
        api_url: str = MAERKI_BAUMANN_API_URL,
    ...
    def search(self, request: LinkedInSearchRequest) -> ParserSearchResponse:
        try:
            with httpx.Client(
                headers={**MAERKI_BAUMANN_HEADERS, "Referer": self.base_url},
        ...
        except MaerkiBaumannParseError:
            raise
        except httpx.HTTPError as exc:
            raise DirectCompanyRequestError("Maerki Baumann vacancy request failed") from exc
        except (json.JSONDecodeError, TypeError, ValueError) as exc:
            raise DirectCompanyRequestError("Maerki Baumann vacancy parsing failed") from exc

        jobs = [self.normalize_job(record) for record in records]
        if request.deduplicate:
            jobs = deduplicate_maerki_baumann_jobs(jobs)
        return ParserSearchResponse(
            parser=self.parser_id,
            status="completed",
            search_url=self.base_url,
            jobs=jobs,
            message=(f"Scanned {len(jobs)} Maerki Baumann vacancies from the official catalog"),
        )
    ...
    def enrich_records(
        ...
        def fetch_detail(
            ...
            except (httpx.HTTPError, MaerkiBaumannParseError, ValueError) as exc:
...
def parse_catalog_payload(payload: Any) -> list[dict[str, Any]]:
    if not isinstance(payload, dict) or not isinstance(payload.get("jobs"), list):
        raise MaerkiBaumannParseError("Maerki Baumann catalog payload is malformed")
    ...
    if error_message:
        raise MaerkiBaumannParseError(f"Maerki Baumann catalog returned an error: {error_message}")

    records: list[dict[str, Any]] = []
    seen_ids: set[str] = set()
    for raw_record in payload["jobs"]:
        if not isinstance(raw_record, dict):
            raise MaerkiBaumannParseError("Maerki Baumann catalog contains an invalid vacancy")
        ...
        if (
            ...
            raise MaerkiBaumannParseError(
                "Maerki Baumann catalog contains an incomplete or non-Swiss vacancy"
            )
        if job_id in seen_ids:
            raise MaerkiBaumannParseError("Maerki Baumann catalog contains duplicate vacancy IDs")
        seen_ids.add(job_id)
        record["id"] = job_id
        record["title"] = title
        record["city"] = city
...
def parse_detail_html(
    ...
    *,
    ...
    if not is_detail_url(page_url):
        raise MaerkiBaumannParseError("Maerki Baumann detail page returned a different vacancy")

    page = Selector(page_html)
    schemas = list(extract_job_posting_schemas(page))
    if len(schemas) != 1:
        raise MaerkiBaumannParseError("Maerki Baumann detail page is missing its JobPosting data")
    schema = schemas[0]
    ...
    if (
        ...
        or not location
        or locality != optional_text(expected_city)
        or not description
        or not apply_url
    ):
        raise MaerkiBaumannParseError(
            "Maerki Baumann detail page contains an incomplete or mismatched vacancy"
        )
...
def extract_swiss_location(value: Any) -> dict[str, str] | None:
    ...
    for location in locations:
        ...
        return {
            "locality": locality,
            "label": f"{prefix}{locality}, Switzerland",
        }
    return None
...
def extract_apply_urls(description_html: str, *, page_url: str) -> set[str]:
    ...
    return {
        ...
        if (value := optional_text(raw_value)) and is_apply_url(value, page_url=page_url)
    }
...
def is_detail_url(value: Any) -> bool:
    ...
    return (
        parts.scheme == "https"
        and parts.netloc.casefold() == "jobs.maerki-baumann.ch"
...
def is_apply_url(value: Any, *, page_url: str) -> bool:
    ...
    source_values = dict(part.split("=", 1) for part in parts.query.split("&") if "=" in part)
    return (
        parts.scheme == "https"
        and parts.netloc.casefold() == "jobs.maerki-baumann.ch"
...
def deduplicate_maerki_baumann_jobs(jobs: Iterable[ParsedJob]) -> list[ParsedJob]:
    seen: set[str] = set()
