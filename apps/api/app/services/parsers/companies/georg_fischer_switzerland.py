...
class GeorgFischerSwitzerlandParseError(DirectCompanyRequestError):
    pass


class GeorgFischerSwitzerlandJobsParser:
    """Collect Georg Fischer vacancies from Workday's Switzerland facet."""

    parser_id = "georg_fischer_switzerland"

    def __init__(
        self,
        *,
        base_url: str = GEORG_FISCHER_SWITZERLAND_JOBS_BASE_URL,
    ...
    def api_base_url(self) -> str:
        ...
        return (
            f"{parts.scheme}://{parts.netloc}/wday/cxs/{GEORG_FISCHER_TENANT}/{GEORG_FISCHER_SITE}"
        )

    @property
    def site_base_url(self) -> str:
        parts = urlsplit(self.base_url)
        return f"{parts.scheme}://{parts.netloc}/{GEORG_FISCHER_SITE}"
    ...
    def search(self, request: LinkedInSearchRequest) -> ParserSearchResponse:
        ...
        except GeorgFischerSwitzerlandParseError:
            raise
        except httpx.HTTPError as exc:
            raise DirectCompanyRequestError(
                "Georg Fischer Switzerland Workday request failed"
            ) from exc
        except (TypeError, ValueError) as exc:
            raise DirectCompanyRequestError(
                "Georg Fischer Switzerland vacancy parsing failed"
            ) from exc
        ...
        if request.deduplicate:
            jobs = deduplicate_georg_fischer_switzerland_jobs(jobs)
        return ParserSearchResponse(
            ...
            jobs=jobs,
            message=(
                f"Scanned {len(jobs)} verified Georg Fischer Switzerland vacancies "
                f"from {total} Workday facet records across {pages_fetched} page requests"
            ),
        )
    ...
    def collect_listing_records(
        ...
        *,
        ...
        for offset in offsets:
            ...
            if total is None:
                ...
                if len(offsets) > self.max_pages:
                    raise GeorgFischerSwitzerlandParseError(
                        f"Georg Fischer Switzerland exposes {len(offsets)} pages, "
                        f"above the configured limit of {self.max_pages}"
                    )
            elif page_total not in {0, total}:
                raise GeorgFischerSwitzerlandParseError(
                    "Georg Fischer Switzerland changed its vacancy total during pagination"
                )

            if offset < (total or 0) and not postings:
                raise GeorgFischerSwitzerlandParseError(
                    "Georg Fischer Switzerland Workday page at offset "
                    f"{offset} was unexpectedly empty"
                )
            ...
            for posting in postings:
                ...
                if not is_safe_external_path(external_path):
                    raise GeorgFischerSwitzerlandParseError(
                        "Georg Fischer Switzerland returned an unsafe vacancy path"
                    )
        ...
        if len(records) != (total or 0):
            raise GeorgFischerSwitzerlandParseError(
                f"Georg Fischer Switzerland yielded {len(records)} unique vacancies "
        ...
        def fetch_detail(record: dict[str, Any]) -> tuple[dict[str, Any], Any]:
            ...
            try:
                ...
                if not isinstance(payload, dict):
                    raise GeorgFischerSwitzerlandParseError(
                        "Georg Fischer detail response must be an object"
                    )
                return record, payload
            except (httpx.HTTPError, ValueError, GeorgFischerSwitzerlandParseError) as exc:
    ...
    def normalize_job(self, record: dict[str, Any]) -> ParsedJob:
        ...
        return ParsedJob(
            ...
            company=optional_text(organization.get("name")) or "Georg Fischer",
...
def parse_listing_payload(payload: Any) -> tuple[int, list[dict[str, Any]]]:
    if not isinstance(payload, dict):
        raise GeorgFischerSwitzerlandParseError(
            "Georg Fischer Switzerland Workday response must be an object"
        )
    total = payload.get("total")
    if isinstance(total, bool) or not isinstance(total, int) or total < 0:
        raise GeorgFischerSwitzerlandParseError(
            "Georg Fischer Switzerland Workday response has an invalid total"
        )
    postings = payload.get("jobPostings")
    if not isinstance(postings, list):
        raise GeorgFischerSwitzerlandParseError(
            "Georg Fischer Switzerland Workday response has invalid jobPostings"
        )
    ...
    for posting in postings:
        if (
            ...
            raise GeorgFischerSwitzerlandParseError(
                "Georg Fischer Switzerland Workday response contains an incomplete vacancy"
            )
...
def is_public_job_url(value: Any, *, expected_host: str) -> bool:
    ...
    return bool(
        ...
        and parts.path.startswith(f"/{GEORG_FISCHER_SITE}/job/")
        and not parts.fragment
    )


def deduplicate_georg_fischer_switzerland_jobs(
