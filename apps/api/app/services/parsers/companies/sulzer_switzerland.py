...
class SulzerSwitzerlandParseError(DirectCompanyRequestError):
    pass


class SulzerSwitzerlandJobsParser:
    """Collect every Swiss vacancy from Sulzer's public Workday site."""

    parser_id = "sulzer_switzerland"

    def __init__(
        self,
        *,
        base_url: str = SULZER_SWITZERLAND_JOBS_BASE_URL,
    ...
    def api_base_url(self) -> str:
        ...
        return f"{parts.scheme}://{parts.netloc}/wday/cxs/{SULZER_TENANT}/{SULZER_SITE}"

    @property
    def site_base_url(self) -> str:
        parts = urlsplit(self.base_url)
        return f"{parts.scheme}://{parts.netloc}/{SULZER_SITE}"
    ...
    def search(self, request: LinkedInSearchRequest) -> ParserSearchResponse:
        ...
        except SulzerSwitzerlandParseError:
            raise
        except httpx.HTTPError as exc:
            raise DirectCompanyRequestError("Sulzer Switzerland Workday request failed") from exc
        except Exception as exc:
            raise DirectCompanyRequestError("Sulzer Switzerland vacancy request failed") from exc

        jobs = [self.normalize_job(record) for record in records]
        if request.deduplicate:
            jobs = deduplicate_sulzer_switzerland_jobs(jobs)
        return ParserSearchResponse(
            ...
            jobs=jobs,
            message=(
                f"Scanned {len(jobs)} Sulzer Switzerland vacancies from {total} "
                f"catalog records across {pages_fetched} Workday page requests"
            ),
        )
    ...
    def collect_listing_records(
        ...
        *,
        ...
        for offset in offsets:
            ...
            if total is None:
                ...
                if len(offsets) > self.max_pages:
                    raise SulzerSwitzerlandParseError(
                        f"Sulzer Switzerland exposes {len(offsets)} pages, above "
                        f"the configured limit of {self.max_pages}"
                    )
            elif page_total != total:
                raise SulzerSwitzerlandParseError(
                    "Sulzer Switzerland changed its vacancy total during pagination"
                )

            if offset < (total or 0) and not postings:
                raise SulzerSwitzerlandParseError(
                    f"Sulzer Switzerland Workday page at offset {offset} was unexpectedly empty"
                )
        ...
        if len(records) != (total or 0):
            raise SulzerSwitzerlandParseError(
                f"Sulzer Switzerland yielded {len(records)} unique vacancies of "
    ...
    def normalize_job(self, record: dict[str, Any]) -> ParsedJob:
        ...
        return ParsedJob(
            ...
            company=optional_text(organization.get("name")) or "Sulzer",
...
def parse_listing_payload(payload: Any) -> tuple[int, list[dict[str, Any]]]:
    if not isinstance(payload, dict):
        raise SulzerSwitzerlandParseError("Sulzer Switzerland Workday response must be an object")
    total = payload.get("total")
    if isinstance(total, bool) or not isinstance(total, int) or total < 0:
        raise SulzerSwitzerlandParseError(
            "Sulzer Switzerland Workday response has an invalid total"
        )
    postings = payload.get("jobPostings")
    if not isinstance(postings, list):
        raise SulzerSwitzerlandParseError(
            "Sulzer Switzerland Workday response has invalid jobPostings"
        )
    ...
    for posting in postings:
        if (
            ...
            raise SulzerSwitzerlandParseError(
                "Sulzer Switzerland Workday response contains an incomplete vacancy"
            )
...
def deduplicate_sulzer_switzerland_jobs(
