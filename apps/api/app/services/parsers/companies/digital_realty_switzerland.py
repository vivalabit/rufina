...
DIGITAL_REALTY_SWITZERLAND_JOBS_BASE_URL = (
    "https://hdep.fa.us2.oraclecloud.com/hcmUI/CandidateExperience/en/sites/CX/jobs?"
    "location=Switzerland&locationId=300000000361301&locationLevel=country&"
    "mode=job-location"
)
DIGITAL_REALTY_SWITZERLAND_JOBS_API_URL = (
    "https://hdep.fa.us2.oraclecloud.com/hcmRestApi/resources/latest"
)
DIGITAL_REALTY_SITE_NUMBER = "CX"
DIGITAL_REALTY_SWITZERLAND_LOCATION_ID = "300000000361301"
DIGITAL_REALTY_RESULTS_PER_PAGE = 25
DIGITAL_REALTY_HEADERS = {
...
class DigitalRealtySwitzerlandParseError(DirectCompanyRequestError):
    pass


class DigitalRealtySwitzerlandJobsParser:
    """Collect Digital Realty vacancies available in Switzerland from Oracle HCM."""

    parser_id = "digital_realty_switzerland"

    def __init__(
        self,
        *,
        base_url: str = DIGITAL_REALTY_SWITZERLAND_JOBS_BASE_URL,
        api_url: str = DIGITAL_REALTY_SWITZERLAND_JOBS_API_URL,
    ...
    def search(self, request: LinkedInSearchRequest) -> ParserSearchResponse:
        try:
            with httpx.Client(
                headers={**DIGITAL_REALTY_HEADERS, "Referer": self.base_url},
                ...
                records = [
                    record for record in records if is_swiss_vacancy(record.get("detail") or record)
                ]
        except DigitalRealtySwitzerlandParseError:
            raise
        except httpx.HTTPError as exc:
            raise DirectCompanyRequestError(
                "Digital Realty Switzerland vacancy request failed"
            ) from exc
        except (TypeError, ValueError) as exc:
            raise DirectCompanyRequestError(
                "Digital Realty Switzerland vacancy parsing failed"
            ) from exc

        jobs = [self.normalize_job(record) for record in records]
        if request.deduplicate:
            jobs = deduplicate_digital_realty_jobs(jobs)
        return ParserSearchResponse(
            ...
            jobs=jobs,
            message=(
                f"Scanned {len(jobs)} Digital Realty Switzerland vacancies from "
                f"{total} Swiss catalog "
    ...
    def fetch_listing_page(
        ...
        *,
        ...
        finder = ",".join(
            (
                f"siteNumber={DIGITAL_REALTY_SITE_NUMBER}",
                f"selectedLocationsFacet={DIGITAL_REALTY_SWITZERLAND_LOCATION_ID}",
                f"limit={DIGITAL_REALTY_RESULTS_PER_PAGE}",
                f"offset={offset}",
        ...
        if page_offset != offset:
            raise DigitalRealtySwitzerlandParseError(
                f"Digital Realty Switzerland returned offset {page_offset} for requested offset {offset}"
            )
    ...
    def collect_listing_records(
        ...
        for catalog_pass in range(1, self.max_catalog_passes + 1):
            ...
            required_pages = max(1, ceil(expected_total / DIGITAL_REALTY_RESULTS_PER_PAGE))
            if required_pages > self.max_pages:
                raise DigitalRealtySwitzerlandParseError(
                    f"Digital Realty Switzerland exposes {required_pages} pages, above the configured "
                    f"limit of {self.max_pages}"
                )

            pages = [first_page]
            for offset in range(
                DIGITAL_REALTY_RESULTS_PER_PAGE,
                expected_total,
                DIGITAL_REALTY_RESULTS_PER_PAGE,
            ):
        ...
        raise DigitalRealtySwitzerlandParseError(
            f"Digital Realty Switzerland yielded only {last_unique} unique vacancies of {last_total} "
    ...
    def enrich_records(
        ...
        def fetch_detail(
            ...
            try:
                response = client.get(
                    ...
                    params={
                        "expand": "all",
                        "onlyData": "true",
                        "finder": f"ById;Id={job_id},siteNumber={DIGITAL_REALTY_SITE_NUMBER}",
                    },
                )
                response.raise_for_status()
            ...
            except (httpx.HTTPError, DigitalRealtySwitzerlandParseError, ValueError) as exc:
...
def parse_listing_payload(payload: Any) -> tuple[int, int, list[dict[str, Any]]]:
    if not isinstance(payload, dict):
        raise DigitalRealtySwitzerlandParseError(
            "Digital Realty Switzerland jobs response must be an object"
        )
    items = payload.get("items")
    if not isinstance(items, list) or len(items) != 1 or not isinstance(items[0], dict):
        raise DigitalRealtySwitzerlandParseError(
            "Digital Realty Switzerland jobs response has invalid search results"
        )
    result = items[0]
    ...
    if isinstance(total, bool) or not isinstance(total, int) or total < 0:
        raise DigitalRealtySwitzerlandParseError(
            "Digital Realty Switzerland jobs response has an invalid total"
        )
    if isinstance(offset, bool) or not isinstance(offset, int) or offset < 0:
        raise DigitalRealtySwitzerlandParseError(
            "Digital Realty Switzerland jobs response has an invalid offset"
        )
    if not isinstance(records, list):
        raise DigitalRealtySwitzerlandParseError(
            "Digital Realty Switzerland jobs response has invalid vacancies"
        )
    ...
    for item in records:
        if (
            ...
            raise DigitalRealtySwitzerlandParseError(
                "Digital Realty Switzerland jobs response contains an incomplete vacancy"
            )
...
def parse_detail_payload(payload: Any, *, expected_job_id: str) -> dict[str, Any]:
    if not isinstance(payload, dict):
        raise DigitalRealtySwitzerlandParseError(
            "Digital Realty Switzerland detail response must be an object"
        )
    items = payload.get("items")
    if not isinstance(items, list) or len(items) != 1 or not isinstance(items[0], dict):
        raise DigitalRealtySwitzerlandParseError(
            "Digital Realty Switzerland detail response has invalid vacancy data"
        )
    detail = items[0]
    if (
        ...
        raise DigitalRealtySwitzerlandParseError(
            "Digital Realty Switzerland detail response returned another vacancy"
        )
    return detail
    ...
    for key in ("secondaryLocations", "otherWorkLocations", "workLocation"):
        ...
        for location in locations:
            ...
            if first_text(location.get("CountryCode"), location.get("Country")) == "CH":
                return True
            if optional_text(location.get("GeographyId")) == DIGITAL_REALTY_SWITZERLAND_LOCATION_ID:
                return True
    return False
...
def extract_description(record: dict[str, Any]) -> str | None:
    ...
    for label, key in (
        ...
        ("About Digital Realty", "CorporateDescriptionStr"),
    ):
...
def deduplicate_digital_realty_jobs(jobs: Iterable[ParsedJob]) -> list[ParsedJob]:
    seen: set[str] = set()
