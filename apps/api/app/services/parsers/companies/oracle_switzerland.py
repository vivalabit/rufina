...
ORACLE_SWITZERLAND_JOBS_BASE_URL = (
...
ORACLE_SWITZERLAND_JOBS_API_URL = "https://eeho.fa.us2.oraclecloud.com/hcmRestApi/resources/latest"
ORACLE_SITE_NUMBER = "CX_45001"
ORACLE_SWITZERLAND_LOCATION_ID = "300000000106764"
ORACLE_RESULTS_PER_PAGE = 25
ORACLE_HEADERS = {
...
class OracleSwitzerlandParseError(DirectCompanyRequestError):
    pass


class OracleSwitzerlandJobsParser:
    """Collect Oracle vacancies available in Switzerland from Oracle HCM."""

    parser_id = "oracle_switzerland"

    def __init__(
        self,
        *,
        base_url: str = ORACLE_SWITZERLAND_JOBS_BASE_URL,
        api_url: str = ORACLE_SWITZERLAND_JOBS_API_URL,
    ...
    def search(self, request: LinkedInSearchRequest) -> ParserSearchResponse:
        try:
            with httpx.Client(
                headers={**ORACLE_HEADERS, "Referer": self.base_url},
        ...
        except OracleSwitzerlandParseError:
            raise
        except httpx.HTTPError as exc:
            raise DirectCompanyRequestError("Oracle Switzerland vacancy request failed") from exc
        except (TypeError, ValueError) as exc:
            raise DirectCompanyRequestError("Oracle Switzerland vacancy parsing failed") from exc

        jobs = [self.normalize_job(record) for record in records]
        if request.deduplicate:
            jobs = deduplicate_oracle_switzerland_jobs(jobs)
        return ParserSearchResponse(
            ...
            jobs=jobs,
            message=(
                f"Scanned {len(jobs)} Oracle Switzerland vacancies from {total} "
    ...
    def fetch_listing_page(
        ...
        *,
        ...
        finder = ",".join(
            (
                f"siteNumber={ORACLE_SITE_NUMBER}",
                f"limit={ORACLE_RESULTS_PER_PAGE}",
                f"offset={offset}",
                f"locationId={ORACLE_SWITZERLAND_LOCATION_ID}",
                f"selectedLocationsFacet={ORACLE_SWITZERLAND_LOCATION_ID}",
            )
        )
        response = client.get(
        ...
        if page_offset != offset:
            raise OracleSwitzerlandParseError(
                f"Oracle returned offset {page_offset} for requested offset {offset}"
            )
    ...
    def collect_listing_records(
        ...
        for catalog_pass in range(self.max_catalog_passes):
            ...
            required_pages = max(1, ceil(expected_total / ORACLE_RESULTS_PER_PAGE))
            if required_pages > self.max_pages:
                raise OracleSwitzerlandParseError(
                    f"Oracle exposes {required_pages} pages, above the configured "
                    f"limit of {self.max_pages}"
                )

            pages = [first_page]
            for offset in range(
                ORACLE_RESULTS_PER_PAGE,
                expected_total,
                ORACLE_RESULTS_PER_PAGE,
            ):
        ...
        raise OracleSwitzerlandParseError(
            f"Oracle yielded only {last_unique} unique vacancies of {last_total} "
            ...
            try:
                response = client.get(
                    ...
                    params={
                        "expand": "all",
                        "onlyData": "true",
                        "finder": (f'ById;Id="{job_id}",siteNumber={ORACLE_SITE_NUMBER}'),
                    },
                )
                response.raise_for_status()
                return record, parse_detail_payload(response.json(), expected_job_id=job_id)
            except (httpx.HTTPError, OracleSwitzerlandParseError, ValueError) as exc:
    ...
    def normalize_job(self, record: dict[str, Any]) -> ParsedJob:
        ...
        job_id = optional_text(record.get("Id"))
        public_url = build_job_url(job_id) if job_id else self.base_url
...
def parse_listing_payload(payload: Any) -> tuple[int, int, list[dict[str, Any]]]:
    if not isinstance(payload, dict):
        raise OracleSwitzerlandParseError("Oracle jobs response must be an object")
    items = payload.get("items")
    if not isinstance(items, list) or len(items) != 1 or not isinstance(items[0], dict):
        raise OracleSwitzerlandParseError("Oracle jobs response has invalid search results")
    result = items[0]
    total = result.get("TotalJobsCount")
    offset = result.get("Offset")
    records = result.get("requisitionList")
    if isinstance(total, bool) or not isinstance(total, int) or total < 0:
        raise OracleSwitzerlandParseError("Oracle jobs response has an invalid total")
    if isinstance(offset, bool) or not isinstance(offset, int) or offset < 0:
        raise OracleSwitzerlandParseError("Oracle jobs response has an invalid offset")
    if not isinstance(records, list):
        raise OracleSwitzerlandParseError("Oracle jobs response has invalid vacancies")
    ...
    for item in records:
        if (
            ...
            raise OracleSwitzerlandParseError(
                "Oracle jobs response contains an incomplete or non-Swiss vacancy"
            )
...
def parse_detail_payload(payload: Any, *, expected_job_id: str) -> dict[str, Any]:
    if not isinstance(payload, dict):
        raise OracleSwitzerlandParseError("Oracle detail response must be an object")
    items = payload.get("items")
    if not isinstance(items, list) or len(items) != 1 or not isinstance(items[0], dict):
        raise OracleSwitzerlandParseError("Oracle detail response has invalid vacancy data")
    detail = items[0]
    if optional_text(detail.get("Id")) != expected_job_id or not optional_text(detail.get("Title")):
        raise OracleSwitzerlandParseError("Oracle detail response returned another vacancy")
    return detail
    ...
    for key in ("secondaryLocations", "otherWorkLocations", "workLocation"):
        ...
        for location in locations:
            if not isinstance(location, dict):
                continue
            if optional_text(location.get("CountryCode")) == "CH":
                return True
            if optional_text(location.get("GeographyId")) == ORACLE_SWITZERLAND_LOCATION_ID:
                return True
    return False
...
def extract_description(record: dict[str, Any]) -> str | None:
    ...
    for label, key in (
        ...
        ("About Oracle", "CorporateDescriptionStr"),
    ):
...
def build_job_url(job_id: str) -> str:
    return f"https://careers.oracle.com/en/sites/jobsearch/job/{job_id}"


def deduplicate_oracle_switzerland_jobs(
