...
class BossardSwitzerlandParseError(DirectCompanyRequestError):
    pass


class BossardSwitzerlandJobsParser:
    """Collect Bossard vacancies from Workday's Zug location facet."""

    parser_id = "bossard_switzerland"

    def __init__(
        self,
        *,
        base_url: str = BOSSARD_SWITZERLAND_JOBS_BASE_URL,
    ...
    def api_base_url(self) -> str:
        ...
        return f"{parts.scheme}://{parts.netloc}/wday/cxs/{BOSSARD_TENANT}/{BOSSARD_SITE}"

    @property
    def site_base_url(self) -> str:
        parts = urlsplit(self.base_url)
        return f"{parts.scheme}://{parts.netloc}/{BOSSARD_SITE}"
    ...
    def search(self, request: LinkedInSearchRequest) -> ParserSearchResponse:
        ...
        except BossardSwitzerlandParseError:
            raise
        except httpx.HTTPError as exc:
            raise DirectCompanyRequestError("Bossard Switzerland Workday request failed") from exc
        except (TypeError, ValueError) as exc:
            raise DirectCompanyRequestError("Bossard Switzerland vacancy parsing failed") from exc
        ...
        if request.deduplicate:
            jobs = deduplicate_bossard_switzerland_jobs(jobs)
        return ParserSearchResponse(
            ...
            jobs=jobs,
            message=(
                f"Scanned {len(jobs)} verified Bossard Switzerland vacancies "
                f"from {total} Workday Zug facet records across "
        ...
        *,
        ...
        for offset in offsets:
            response = client.post(
                ...
                json={
                    "appliedFacets": {
                        "locations": [BOSSARD_ZUG_LOCATION_FACET],
                    },
            ...
            if total is None:
                validate_zug_facet(payload, page_total)
                total = page_total
                offsets.extend(page_offsets(total)[1:])
                if len(offsets) > self.max_pages:
                    raise BossardSwitzerlandParseError(
                        f"Bossard Switzerland exposes {len(offsets)} pages, "
                        f"above the configured limit of {self.max_pages}"
                    )
            elif page_total not in {0, total}:
                raise BossardSwitzerlandParseError(
                    "Bossard Switzerland changed its vacancy total during pagination"
                )

            if offset < (total or 0) and not postings:
                raise BossardSwitzerlandParseError(
                    f"Bossard Switzerland Workday page at offset {offset} was unexpectedly empty"
                )
            ...
            for posting in postings:
                ...
                if not is_safe_external_path(external_path):
                    raise BossardSwitzerlandParseError(
                        "Bossard Switzerland returned an unsafe vacancy path"
                    )
        ...
        if len(records) != (total or 0):
            raise BossardSwitzerlandParseError(
                f"Bossard Switzerland yielded {len(records)} unique vacancies "
        ...
        def fetch_detail(record: dict[str, Any]) -> tuple[dict[str, Any], Any]:
            ...
            try:
                ...
                if not isinstance(payload, dict):
                    raise BossardSwitzerlandParseError("Bossard detail response must be an object")
                return record, payload
            except (httpx.HTTPError, ValueError, BossardSwitzerlandParseError) as exc:
    ...
    def normalize_job(self, record: dict[str, Any]) -> ParsedJob:
        ...
        return ParsedJob(
            ...
            company=optional_text(organization.get("name")) or "Bossard AG",
...
def parse_listing_payload(payload: Any) -> tuple[int, list[dict[str, Any]]]:
    if not isinstance(payload, dict):
        raise BossardSwitzerlandParseError("Bossard Switzerland Workday response must be an object")
    total = payload.get("total")
    if isinstance(total, bool) or not isinstance(total, int) or total < 0:
        raise BossardSwitzerlandParseError(
            "Bossard Switzerland Workday response has an invalid total"
        )
    postings = payload.get("jobPostings")
    if not isinstance(postings, list):
        raise BossardSwitzerlandParseError(
            "Bossard Switzerland Workday response has invalid jobPostings"
        )
    ...
    for posting in postings:
        if (
            ...
            raise BossardSwitzerlandParseError(
                "Bossard Switzerland Workday response contains an incomplete vacancy"
            )
        records.append(posting)
    return total, records


def validate_zug_facet(payload: Any, total: int) -> None:
    if total == 0:
        return
    facets = payload.get("facets") if isinstance(payload, dict) else None
    if not isinstance(facets, list):
        raise BossardSwitzerlandParseError(
            "Bossard Switzerland Workday response is missing the Zug facet"
        )

    for facet in facets:
        if not isinstance(facet, dict) or facet.get("facetParameter") != "locationMainGroup":
            continue
        groups = facet.get("values")
        if not isinstance(groups, list):
            continue
        for group in groups:
            if not isinstance(group, dict) or group.get("facetParameter") != "locations":
                continue
            ...
            for value in values:
                if not isinstance(value, dict):
                    continue
                if optional_text(value.get("id")) != BOSSARD_ZUG_LOCATION_FACET:
                    continue
                descriptor = optional_text(value.get("descriptor"))
                count = value.get("count")
                if descriptor == "Zug" and count == total:
                    return
    raise BossardSwitzerlandParseError(
        "Bossard Switzerland Workday response did not confirm the requested Zug facet"
    )
...
def extract_employment_type(
    ...
    values = [
        optional_text(posting_info.get("timeType")) or optional_text(listing.get("timeType")),
...
def is_public_job_url(value: Any, *, expected_host: str) -> bool:
    ...
    return bool(
        ...
        and parts.path.startswith(f"/{BOSSARD_SITE}/job/")
        and not parts.fragment
    )


def deduplicate_bossard_switzerland_jobs(
