...
SANOFI_SWITZERLAND_COUNTRY = "Switzerland"
SANOFI_RESULTS_PER_PAGE = 15
SANOFI_HEADERS = {
...
class SanofiSwitzerlandParseError(DirectCompanyRequestError):
    pass


class SanofiSwitzerlandJobsParser:
    """Collect Sanofi's complete official country facet for Switzerland."""

    parser_id = "sanofi_switzerland"

    def __init__(
        self,
        *,
        base_url: str = SANOFI_SWITZERLAND_JOBS_BASE_URL,
    ...
    def listing_endpoint(self) -> str:
        return f"{self.site_origin}/en/search-jobs/resultspost"

    def search(self, request: LinkedInSearchRequest) -> ParserSearchResponse:
        try:
            with httpx.Client(
                headers={**SANOFI_HEADERS, "Origin": self.site_origin, "Referer": self.base_url},
        ...
        if request.deduplicate:
            jobs = deduplicate_sanofi_jobs(jobs)
        return ParserSearchResponse(
            ...
            jobs=jobs,
            message=(
                f"Scanned {len(jobs)} verified Sanofi Switzerland vacancies from "
    ...
    def collect_listing_records(
        ...
        for catalog_pass in range(1, self.max_catalog_passes + 1):
            ...
            if total_pages > self.max_pages:
                raise SanofiSwitzerlandParseError(
                    f"Sanofi Switzerland exposes {total_pages} pages, above the "
        ...
        raise SanofiSwitzerlandParseError(
            f"Sanofi Switzerland yielded {last_unique_count} unique vacancies of "
    ...
    def enrich_records(
        ...
        def fetch_detail(
            ...
            except (httpx.HTTPError, SanofiSwitzerlandParseError, ValueError) as exc:
...
def listing_payload(
    *,
    ...
    return {
        "ActiveFacetID": SANOFI_SWITZERLAND_FACET_ID,
        "CurrentPage": page,
        "RecordsPerPage": SANOFI_RESULTS_PER_PAGE,
        ...
        "FacetFilters": [
            {
                ...
                "FacetType": 2,
                "Count": known_total,
                "Display": SANOFI_SWITZERLAND_COUNTRY,
                "IsApplied": True,
                "FieldName": "",
            }
        ],
        ...
        "LocationType": "",
        "LocationPath": "",
        "OrganizationIds": SANOFI_ORGANIZATION_ID,
        "RefinedKeywords": [],
        "PostalCode": "",
        "ResultsType": 0,
    }
...
def parse_listing_response(
    ...
    *,
    ...
    if not isinstance(payload, dict):
        raise SanofiSwitzerlandParseError("Sanofi listing response must be an object")
    ...
    if not isinstance(results_html, str) or not results_html.strip():
        raise SanofiSwitzerlandParseError("Sanofi listing response is missing results HTML")
    if not isinstance(filters_html, str) or not filters_html.strip():
        raise SanofiSwitzerlandParseError("Sanofi listing response is missing filters HTML")

    page = Selector(results_html)
    sections = page.css("section#search-results")
    if len(sections) != 1:
        raise SanofiSwitzerlandParseError("Sanofi listing has an invalid results section")
    section = sections[0]
    ...
    if (
        ...
        or page_size != SANOFI_RESULTS_PER_PAGE
        or active_facet != SANOFI_SWITZERLAND_FACET_ID
        or organization != SANOFI_ORGANIZATION_ID
    ):
    ...
    for card in section.css("#search-results-list > ul > li"):
        ...
        if len(links) != 1:
            raise SanofiSwitzerlandParseError("Sanofi listing contains an invalid job card")
        link = links[0]
        job_id = optional_text(link.css("::attr(data-job-id)").get()) or ""
        path = optional_text(link.css("::attr(href)").get())
        title = optional_text(" ".join(link.css("h2 ::text").getall()))
        if title is None:
            title = optional_text(" ".join(link.css("h2::text").getall()))
        ...
        if (
            ...
            or not path
            or not path.startswith("/en/job/")
            or not title
            or not location
            or SANOFI_SWITZERLAND_COUNTRY.casefold() not in location.casefold()
            or button_ids != {job_id}
        ):
            raise SanofiSwitzerlandParseError(
                "Sanofi listing contains an incomplete or non-Swiss vacancy"
            )
        records.append(
            {
                "job_id": job_id,
                ...
                "url": urljoin("https://jobs.sanofi.com", path),
            }
        )

    expected_count = min(
        SANOFI_RESULTS_PER_PAGE,
        max(0, total - ((expected_page - 1) * SANOFI_RESULTS_PER_PAGE)),
    )
    if len(records) != expected_count:
        raise SanofiSwitzerlandParseError(
            f"Sanofi listing page {expected_page} returned {len(records)} of "
    ...
    if not isinstance(has_jobs, bool) or has_jobs is not (total > 0):
        raise SanofiSwitzerlandParseError("Sanofi listing has invalid hasJobs metadata")
...
def validate_switzerland_filter(filters_html: str, *, expected_total: int) -> None:
    ...
    matches = filters.css(
        ...
        f'[data-id="{SANOFI_SWITZERLAND_FACET_ID}"]'
        f'[data-display="{SANOFI_SWITZERLAND_COUNTRY}"]'
    )
    if len(matches) != 1:
        raise SanofiSwitzerlandParseError("Sanofi response is missing its Switzerland facet")
    checkbox = matches[0]
    ...
    if count != expected_total or checked is None:
        raise SanofiSwitzerlandParseError(
            "Sanofi response did not preserve its Switzerland country facet"
        )
...
def parse_detail_html(page_html: str, *, expected_job_id: str) -> dict[str, Any]:
    ...
    if current_job_id != expected_job_id or not valid_public_job_url(
        ...
        raise SanofiSwitzerlandParseError("Sanofi detail page has invalid job identity")
    if not valid_workday_apply_url(apply_url):
        raise SanofiSwitzerlandParseError("Sanofi detail page has an invalid apply URL")
    ...
    for raw in page.css('script[type="application/ld+json"]::text').getall():
        try:
            ...
            raise SanofiSwitzerlandParseError(
                "Sanofi detail page contains invalid JobPosting JSON"
            ) from exc
    ...
    if (
        ...
        raise SanofiSwitzerlandParseError(
            "Sanofi detail page contains an incomplete or non-Swiss JobPosting"
        )
    detail = dict(posting)
...
def swiss_job_locations(value: Any) -> list[str]:
    ...
    for item in values:
        ...
        if not country or country.casefold() != SANOFI_SWITZERLAND_COUNTRY.casefold():
            continue
        parts = [
...
def valid_public_job_url(value: Any, *, expected_job_id: str) -> bool:
    ...
    return (
        parts.scheme == "https"
        and parts.netloc.casefold() == "jobs.sanofi.com"
        and parts.path.startswith("/en/job/")
        and parts.path.endswith(f"/{SANOFI_ORGANIZATION_ID}/{expected_job_id}")
        and not parts.query
...
def valid_workday_apply_url(value: Any) -> bool:
    ...
    return (
        ...
        and parts.netloc.casefold().startswith("sanofi.")
...
def integer_attribute(node: Any, name: str) -> int:
    ...
    if not value or not re.fullmatch(r"\d+", value):
        raise SanofiSwitzerlandParseError(f"Sanofi response has invalid {name}")
    return int(value)
...
def deduplicate_sanofi_jobs(jobs: Iterable[ParsedJob]) -> list[ParsedJob]:
    seen: set[str] = set()
