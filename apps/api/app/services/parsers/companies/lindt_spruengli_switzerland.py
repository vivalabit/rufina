...
LINDT_SPRUENGLI_SWITZERLAND_JOBS_BASE_URL = (
    "https://lindtspruengli.wd103.myworkdayjobs.com/"
    "LindtSpruengliGroupCareers?"
    "hiringCompany=6ef234644ca31000c2704c6d47960000"
)
LINDT_SPRUENGLI_TENANT = "lindtspruengli"
LINDT_SPRUENGLI_SITE = "LindtSpruengliGroupCareers"
LINDT_SPRUENGLI_SWITZERLAND_HIRING_COMPANY = "6ef234644ca31000c2704c6d47960000"
SWITZERLAND_COUNTRY_FACET = "187134fccb084a0ea9b4b95f23890dbe"
EXPECTED_COMPANY = "Lindt & Sprüngli (Schweiz) AG"
WORKDAY_RESULTS_PER_PAGE = 20
LINDT_SPRUENGLI_SWITZERLAND_HEADERS = {
...
class LindtSpruengliSwitzerlandParseError(DirectCompanyRequestError):
    pass


class LindtSpruengliSwitzerlandJobsParser:
    """Collect the complete Lindt & Sprüngli Switzerland Workday facet."""

    parser_id = "lindt_spruengli_switzerland"

    def __init__(
        self,
        *,
        base_url: str = LINDT_SPRUENGLI_SWITZERLAND_JOBS_BASE_URL,
    ...
    def api_base_url(self) -> str:
        ...
        return (
            ...
            f"{LINDT_SPRUENGLI_TENANT}/{LINDT_SPRUENGLI_SITE}"
        )

    @property
    def site_base_url(self) -> str:
        parts = urlsplit(self.base_url)
        return f"{parts.scheme}://{parts.netloc}/{LINDT_SPRUENGLI_SITE}"
    ...
    def search(self, request: LinkedInSearchRequest) -> ParserSearchResponse:
        ...
        except LindtSpruengliSwitzerlandParseError:
            raise
        except httpx.HTTPError as exc:
            raise DirectCompanyRequestError(
                "Lindt & Sprüngli Switzerland Workday request failed"
            ) from exc
        except (TypeError, ValueError) as exc:
            raise DirectCompanyRequestError(
                "Lindt & Sprüngli Switzerland vacancy parsing failed"
            ) from exc

        verified_records = [record for record in records if is_verified_record(record)]
        jobs = [normalize_job(record, parser=self) for record in verified_records]
        if request.deduplicate:
            jobs = deduplicate_lindt_spruengli_switzerland_jobs(jobs)
        return ParserSearchResponse(
            ...
            jobs=jobs,
            message=(
                f"Scanned {len(jobs)} verified Lindt & Sprüngli Switzerland vacancies "
    ...
    def collect_listing_records(
        ...
        for catalog_pass in range(1, self.max_catalog_passes + 1):
            ...
            validate_hiring_company_facet(first_payload, expected_total=total)
            pages_fetched += 1

            if expected_total is None:
                expected_total = total
            elif total != expected_total:
                raise LindtSpruengliSwitzerlandParseError(
                    "Lindt & Sprüngli Switzerland changed its vacancy total during pagination"
                )
            offsets = page_offsets(total)
            if len(offsets) > self.max_pages:
                raise LindtSpruengliSwitzerlandParseError(
                    f"Lindt & Sprüngli Switzerland exposes {len(offsets)} pages, "
            ...
            for offset in offsets[1:]:
                ...
                expected_page_size = min(WORKDAY_RESULTS_PER_PAGE, total - offset)
                if len(postings) != expected_page_size:
                    raise LindtSpruengliSwitzerlandParseError(
                        f"Lindt & Sprüngli Switzerland Workday page at offset {offset} "
                        "was incomplete"
                    )
            ...
            for offset, postings in page_results:
                for posting in postings:
                    ...
                    if not is_safe_external_path(external_path):
                        raise LindtSpruengliSwitzerlandParseError(
                            "Lindt & Sprüngli Switzerland returned an unsafe vacancy path"
                        )
            ...
            if len(records_by_path) == total:
                return list(records_by_path.values()), total, pages_fetched, catalog_pass
            if len(records_by_path) > total:
                raise LindtSpruengliSwitzerlandParseError(
                    "Lindt & Sprüngli Switzerland catalog changed while it was collected"
                )

        raise LindtSpruengliSwitzerlandParseError(
            f"Lindt & Sprüngli Switzerland yielded {len(records_by_path)} unique "
            f"vacancies of {expected_total or 0} catalog records"
        )
    ...
    def enrich_records(
        ...
        def fetch_detail(
            ...
            try:
                ...
                except LindtSpruengliSwitzerlandParseError as exc:
...
def parse_listing_payload(payload: Any) -> tuple[int, list[dict[str, Any]]]:
    if not isinstance(payload, dict):
        raise LindtSpruengliSwitzerlandParseError(
            "Lindt & Sprüngli Switzerland Workday response must be an object"
        )
    total = payload.get("total")
    postings = payload.get("jobPostings")
    if isinstance(total, bool) or not isinstance(total, int) or total < 0:
        raise LindtSpruengliSwitzerlandParseError(
            "Lindt & Sprüngli Switzerland Workday response has an invalid total"
        )
    if not isinstance(postings, list):
        raise LindtSpruengliSwitzerlandParseError(
            "Lindt & Sprüngli Switzerland Workday response has invalid jobPostings"
        )
    records: list[dict[str, Any]] = []
    for posting in postings:
        if (
            ...
            raise LindtSpruengliSwitzerlandParseError(
                "Lindt & Sprüngli Switzerland Workday response contains an incomplete vacancy"
            )
        records.append(posting)
    return total, records


def validate_hiring_company_facet(payload: Any, *, expected_total: int) -> None:
    if not isinstance(payload, dict) or not isinstance(payload.get("facets"), list):
        raise LindtSpruengliSwitzerlandParseError(
            "Lindt & Sprüngli Switzerland Workday response is missing its facets"
        )
    matches: list[dict[str, Any]] = []
    for facet in payload["facets"]:
        if not isinstance(facet, dict) or facet.get("facetParameter") != "hiringCompany":
            continue
        ...
        matches.extend(
            ...
            and value.get("id") == LINDT_SPRUENGLI_SWITZERLAND_HIRING_COMPANY
        )
    if (
        len(matches) != 1
        or optional_text(matches[0].get("descriptor")) != EXPECTED_COMPANY
        or matches[0].get("count") != expected_total
    ):
        raise LindtSpruengliSwitzerlandParseError(
            "Lindt & Sprüngli Switzerland Workday response is missing its "
            "verified hiring-company facet"
        )
...
def validate_detail_payload(payload: Any, *, expected_path: str) -> None:
    if not isinstance(payload, dict):
        raise LindtSpruengliSwitzerlandParseError(
            "Lindt & Sprüngli Switzerland detail response must be an object"
        )
    ...
    if (
        ...
        raise LindtSpruengliSwitzerlandParseError(
            "Lindt & Sprüngli Switzerland detail response has an unexpected identity"
        )
    external_url = optional_text(posting_info.get("externalUrl"))
    if external_url and urlsplit(external_url).path != f"/{LINDT_SPRUENGLI_SITE}{expected_path}":
        raise LindtSpruengliSwitzerlandParseError(
            "Lindt & Sprüngli Switzerland detail response changed its vacancy path"
        )
...
def normalize_job(
    ...
    *,
    ...
    external_path = optional_text(record.get("externalPath"))
    constructed_url = (
        f"{parser.site_base_url}{external_path}"
        if is_safe_external_path(external_path)
        else parser.base_url
    )
    candidate_url = optional_text(posting_info.get("externalUrl"))
    public_url = (
        candidate_url
        if is_public_job_url(candidate_url, expected_host=urlsplit(parser.base_url).netloc)
...
def is_public_job_url(value: Any, *, expected_host: str) -> bool:
    ...
    return bool(
        ...
        and parts.path.startswith(f"/{LINDT_SPRUENGLI_SITE}/job/")
        and not parts.query
        and not parts.fragment
    )


def deduplicate_lindt_spruengli_switzerland_jobs(
